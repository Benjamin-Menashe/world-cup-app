...
export default function InitTournamentButton() {
  const [status, setStatus] = useState<"idle" | "loading" | "success" | "error">("idle")
  const [message, setMessage] = useState("")

  async function handleInit() {
    if (!confirm("WARNING: This will call the API to fetch all 48 teams and 104 matches for the 2026 World Cup and insert them into the database. Are you sure?")) return;
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setMessage(
          `✅ Initialized perfectly! Added ${s.teamsAdded} Teams and ${s.gamesAdded} Games.` +
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
      ...
      >
        ...
        {status === "loading" ? "Initializing Database..." : "Step 1: Init Tournament Data"}
      </button>
